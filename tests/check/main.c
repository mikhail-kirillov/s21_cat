...
START_TEST(test_getopt_parse_flags_null_args) {
  CLIFlags flags;
  ck_assert_int_eq(
      getopt_parse_flags(0, NULL, kShortGetoptFlags, kLongGetoptFlags, &flags),
      RETURN_CODE_ERROR_NULL);
  ck_assert_int_eq(getopt_parse_flags(0, NULL, NULL, kLongGetoptFlags, &flags),
                   RETURN_CODE_ERROR_NULL);
  ck_assert_int_eq(getopt_parse_flags(0, NULL, kShortGetoptFlags, NULL, &flags),
                   RETURN_CODE_ERROR_NULL);
  ck_assert_int_eq(
      getopt_parse_flags(0, NULL, kShortGetoptFlags, kLongGetoptFlags, NULL),
      RETURN_CODE_ERROR_NULL);
}
END_TEST
...
START_TEST(test_getopt_parse_flags_valid_flags) {
  ...
  ck_assert_int_eq(getopt_parse_flags(argc, argv, kShortGetoptFlags,
                                      kLongGetoptFlags, &flags),
                   RETURN_CODE_OK);
...
START_TEST(test_file_operation_nonexistent_file) {
  CLIFlags flags = {0};
  ck_assert_int_eq(file_operation("definitely_no_such_file_123.txt", &flags),
